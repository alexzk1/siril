...
void adjust_reginfo() {
        ...
        if (com.seq.regparam == NULL || com.seq.regparam[com.cvport] == NULL) {
                ...
                gtk_spin_button_set_value(spin_shifty,
                                -com.seq.regparam[com.cvport][com.seq.current].shifty); //negating visual shifty, so it is calculated as shiftx as Ycurrent - Yref
        }
...
void on_spinbut_shift_value_change(GtkSpinButton *spinbutton, gpointer user_data) {
        ...
        else com.seq.regparam[current_layer][com.seq.current].shifty    = -new_value; //negating visual shifty, so it is calculated as shiftx as Ycurrent - Yref
